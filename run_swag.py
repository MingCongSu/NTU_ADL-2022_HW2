...
class DataTrainingArguments:
    ...
    test_file: Optional[str] = field(
        default=None,
        metadata={
            "help": "An optional input testing data file to evaluate the perplexity on (a json file)."},
    )
    context_file: Optional[str] = field(
        default=None, metadata={"help": "The input context file (a json file)."})
    output_file: Optional[str] = field(
        default=None,
        metadata={"help": "An optional output filename."},
    )
...
def main():
    ...
    if not(os.path.isdir(training_args.output_dir)):
        ...
        os.mkdir(path)
        logger.info(f"ckpt_dir {training_args.output_dir} created")
    ...
    # read context.json
    with open(data_args.context_file, 'r', encoding="utf-8") as read:
        contexts = json.load(read)
    # Preprocessing the datasets.

    def preprocess_function(examples):
        first_sentences = [[context] * 4 for context in examples['question']]
        second_sentences = [[contexts[pid] for pid in paragraphs_id]
                            for paragraphs_id in examples['paragraphs']]

        if 'relevant' in examples.keys():
            labels = [paragraphs_id.index(examples['relevant'][i])
                      for i, paragraphs_id in enumerate(examples['paragraphs'])]
        else:
            labels = [0 for _ in examples["paragraphs"]]
        # else:
        #     labels = [0 for _ in examples['paragraphs']]
        # Flatten out
    ...
    if training_args.do_predict:
        ...
        path = os.path.join(training_args.output_dir, data_args.output_file)
        json.dump(output_json, open(path, 'w',
