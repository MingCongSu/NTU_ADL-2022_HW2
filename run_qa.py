...
def parse_args():
    ...
    parser.add_argument("--do_train_and_valid", action="store_true",
    ...
    parser.add_argument(
        "--context_file", type=str, default=None, help="A csv or a json file containing the context file."
    )
    parser.add_argument(
        "--max_seq_length",
        type=int,
...
def main():
    ...
    with open(args.context_file, 'r', encoding="utf-8") as read:
        contexts = json.load(read)

    ###
    # train
    if args.do_train_and_valid:
        train_relevant = raw_datasets["train"]['relevant']
        train_context = [contexts[i]for i in train_relevant]
        raw_datasets['train'] = raw_datasets['train'].add_column(
            'context', train_context)
        # valid
        valid_relevant = raw_datasets["validation"]['relevant']
        valid_context = [contexts[i]for i in valid_relevant]
        raw_datasets['validation'] = raw_datasets['validation'].add_column(
            'context', valid_context)
        column_names = raw_datasets["train"].column_names
    # predict
    elif args.do_predict:
        test_relevant = raw_datasets["test"]['relevant']
        test_context = [contexts[i]for i in test_relevant]
        raw_datasets['test'] = raw_datasets['test'].add_column(
    ...
    def post_processing_function(examples, features, predictions, stage="eval"):
        ...
        if 'answer' in examples.features:
            references = [{"id": ex["id"], "answer": ex['answer']['text']}
                          for ex in examples]
        else:
            references = [0 for _ in examples]
    ...
    if args.do_train_and_valid:
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                with accelerator.accumulate(model):
                    ...
                    if args.with_tracking:
                        ...
                        # if step % (int(len(train_dataloader)/4)) == 0:
                        #     loss_list.append(loss.detach().float().item())
        ...
        #         'loss_list.csv', header=None, index=None)
        #     print("loss_list saved successfully")
        # Evaluation
        ...
        total = len(prediction.predictions)
        em_list = []
        correct = 0
        for i in range(0, len(prediction.predictions)):
            if(prediction.predictions[i]['prediction_text'] == prediction.label_ids[i]['answer']):
                correct += 1
                if i % (int(len(prediction.predictions)/4)) == 0:
                    em_list.append(correct/total)
        print(f'EM metric:{correct/total}')
        eval_metric = {"EM": correct/total}
        logger.info(f"em: {em_list}")
    ...
    if args.do_predict:
        ...
        #     if(prediction.predictions[i]['prediction_text'] == prediction.label_ids[i]['answer']):
        #         num += 1
        # print(f'EM metric:{num/total}')
        predict_metric = {"EM": num/total}
        ###
    ...
    if args.do_train_and_valid:
        if args.with_tracking:
            ...
            logger.info(f"training log: {log}")
    if args.do_predict:
        # log["squad_v2_predict" if args.version_2_with_negative else "squad_predict"] = predict_metric
        logger.info(json.dumps(predict_metric, indent=4))
        save_prefixed_metrics(predict_metric, args.output_dir)
